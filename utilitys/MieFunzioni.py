...
def find_F_matrix1(x:float,y:float,z:float)-> np.array:
    #Inerzia del satellite
    mi = 4.9048695e3 # km^3/s^2 
    I_3x3=np.eye(3)
    Zero_3x3=np.zeros((3,3))
    ...
    tmp1 = np.hstack((Zero_3x3,I_3x3)) 
    tmp2 = np.hstack((J,Zero_3x3))

    phi = np.vstack((tmp1,tmp2))

    return phi

def find_F_matrix2(omega1:float,omega2:float,omega3:float)-> np.array:
    #Inerzia del satellite
    II1=4500
    II2=4500#kg*m^2, VALORI DA AGGIUSTARE
    II3=4500
    ...
    tmp3 = np.hstack((Zero_3x3,I_3x3))
    tmp4 = np.hstack((Zero_3x3,dfomegadomega))

    phi = np.vstack((tmp3,tmp4))
